...
/**
 * 이 파일은 Node-Media-Server 설치 후, 해당 모듈 경로로 복사
 * 프레임 처리 부분이 추가 되었음
 */
...
class NodeRtmpSession {
  ...
  /**
   * 스트리밍을 시작하는 부분
   * videoQueue에 저장된 영상 패킷을 interval로 전송
   * videoQueue의 내용을 모두 전송하면 세션 종료
   */
  startPublish() {
    this.frameCnt++;
    this.publishTimer = setInterval(() => {
      let rtmpChunks = this.videoQueue.shift();
  ...
  /**
   * FFMPEG에서 전송된 영상 패킷을 처리하는 부분
   * videoQueue에 영상 패킷을 누적
   */
  rtmpVideoHandler() {
    let payload = this.parserPacket.payload.slice(0, this.parserPacket.header.length);
    ...
    this.videoQueue.push(rtmpChunks);
    Logger.log('push queue : ', this.videoQueue.length)
    if(!this.isStartPublishing) {
      this.isStartPublishing = true;
      this.startPublish();
    }
  }
  ...
  rtmpInvokeHandler() {
    ...
    switch (invokeMessage.cmd) {
      ...
      case "deleteStream":
        ...
        Logger.log('=====> DEL STREAM')
        // this.delInvokeMessage = invokeMessage;
  ...
  onDeleteStream(invokeMessage) {
    if (invokeMessage.streamId == this.playStreamId) {
      if (this.isIdling) {
        ...
        let publisherId = context.publishers.get(this.playStreamPath);
        Logger.log('*** publisher Id : ', publisherId)
        if (publisherId != null) {
